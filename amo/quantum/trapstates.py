...
class Trap1D(object):
    ...
    def simulate_spectrum(self, wavevector):
        ...
        spec = occupations * np.abs(trans_els)
        return des, spec
            
    def plot_eigenstates(self, ax, skip=False, is_overlay_potential=True, energy_rescale=None, x_rescale=None, wavefunction_scale=0.3):
        ax.set_xlabel('Distance (um)')
        ax.set_ylabel('Energy (MHz), Re(wavefunction)')
        ax.set_title('Trap Eigenstates')
        ...
        if not skip:
            for i in range(0, self.n_states):
                ax.plot(self.x_values * x_rescale, np.real(self.eigenStates[:, i]) * (wavefunction_scale * energy_rescale / self.n_states * (np.max(self.energies) - np.min(self.energies)) / np.max(np.abs(self.eigenStates[:, i])))
        ...
        if is_overlay_potential:
            ax.plot(self.x_values * x_rescale, self.potential * energy_rescale, color='k', linewidth=2.0)
